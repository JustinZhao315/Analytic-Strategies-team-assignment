...
boxplot(Sal95 ~ Gender, data = data1,
        main = "Salary Distribution by Gender (1995)",
        xlab = "Gender",
        ylab = "Salary (1995)",
        col = c("lightpink", "lightblue"),
        axes = FALSE) # Boxplot Salary vs Gender with full y-axis numbers
axis(1, at = 1:2, labels = c("Female", "Male")) # Custom x-axis
axis(2, at = pretty(data1$Sal95),
     labels = format(pretty(data1$Sal95), big.mark = ",", scientific = FALSE)) # Custom y-axis with no scientific notation
box()
...
ggplot(data1, aes(x = Gender, y = Sal95, fill = Gender)) +
  geom_boxplot() +
  facet_wrap(~ Rank) +
  labs(
    title = "Salary Distribution by Gender within Each Rank (1995)",
    x = "Gender", 
    y = "Salary (1995)"
  ) +
  scale_fill_manual(values = c("lightpink", "lightblue"))
...
ggplot(data1, aes(x = Rank, fill = Gender)) +
  ...
  labs(
    title = "Number of People by Gender within Each Rank，faceted by department",
    x = "Rank",
    y = "proportion"
  ) +
  scale_fill_manual(values = c("lightpink", "lightblue"))+
...
# Multinomial logistic regression for Rank (3 categories)
...
summary(multi_logit)
coefs_1=coef(multi_logit)
exp(coefs_1)
multi_logit_1 <- multinom(Rank ~ Gender,
                        data = data1)
summary(multi_logit_1)
coefs=coef(multi_logit_1)
exp(coefs)

z <- summary(multi_logit_1)$coefficients / summary(multi_logit_1)$standard.errors
p_values_1 <- (1 - pnorm(abs(z), 0, 1)) * 2
p_values_1
summary(multi_logit_1)

# To get p-values (not directly provided in summary)
z <- summary(multi_logit)$coefficients / summary(multi_logit)$standard.errors
p_values <- (1 - pnorm(abs(z), 0, 1)) * 2
p_values
...
par(mfrow = c(1, 1))
cart_gender_sal <- rpart(Sal94 ~ Gender + Exper + Dept + Clin + Cert + Prate,
                     data = data1, method = "anova")
rpart.plot(cart_gender_sal,type=3)
...
p_female <- colMeans(predict(multi_full, newdata = newdata_female, type = "probs"))
p_male   <- colMeans(predict(multi_full, newdata = newdata_male,   type = "probs"))
...
strata <- interaction(data1$Rank, data1$Gender)
train <- sample.split(strata, SplitRatio = 0.7)
